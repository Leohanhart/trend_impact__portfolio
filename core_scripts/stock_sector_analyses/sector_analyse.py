...
from core_scripts.stock_analyses import stock_analyses, stock_analyses_with_ticker
...
We will coppie and past this analyses in Cluster_analyses and add sector and exchange,
type_of_fund to the modules. Good build man.
...
class sector_analyse:
    ...
    def __init__(self):
        ...
        # sets industy
        self.industry       =       []#database_querys.database_querys.get_all_active_industrys()
        
        # sets tickers               
        self.tickers        =       []#database_querys.database_querys.get_all_active_tickers()
    ...
    def create_industy_or_sector_analyses(self, name_industry_or_sector : str = None, 
        ...
        methode : str, optional
            ...
            "DROP"          =   methode where tickers that dont match the length are removed. 
        ...
        # check if varible matches sector or industry and selects tickers
            ...
            self.ticker_selected = database_querys.database_querys.get_all_stocks_with_industry(name_industry_or_sector)
        
        elif name_industry_or_sector in self.sectors:
            
            self.ticker_selected = database_querys.database_querys.get_all_stocks_with_sector(name_industry_or_sector)
...
class sector_analyses_support_funtions(object):
    ...
    def get_analyses(ticker = None, analyeses_name = "", periode = "", name_column_analyses = ""):
        ...
        try:
            ...
            # thows error. 
            raise Exception("Problem with loading analyses", "Sector analyses support function")
...
class cluster_data: 
    ...
    def create_mergede_frame_summed_indicator(  list_tickers : list  = [], 
        ...
        list_tickers = cluster_support_functions.returns_amount_of_valide_tickers(list_tickers=list_tickers, periode = periode)
        main_data = 0 
        main_ticker = 0
        ...
        for i in range(0,len(list_tickers)):
           ...
           list_tickers[i].remove(main_ticker)
        ...
        for i in range(0,len(list_tickers)):
           ...
           # merges the new analyses data with the main
           main_data = merge_data.merge_data(dataframe_main = main_data, new_dataframe= new_data, drop_rate = min_rows)
...
class merge_data:
    ...
    def merge_data( 
        ...
        # check if the new data needs to be cut
        if not merge_support_functions.check_if_main_is_longer(dataframe_main, new_dataframe):
                    
            # tail the data in the right formate 
            new_dataframe= merge_support_functions.tail_the_lenght_in_right_formate(dataframe_new = new_dataframe, data_frame_main=dataframe_main)
         
        # if dataframe is shorter than the main, just drop it.
        else:
...
class merge_support_functions:
    ...
    def merge_two_dataframes( data_frame_main, data_frame_new):
        ...
        """
        # set index data for the index 
        data_index = data_frame_main.index
        
        # unpackdata main data. 
        list_of_data_main = merge_support_functions.unpack_the_dataframe( data_frame = data_frame_main)    
        
        # unpackdata new  data
        list_of_data_new = merge_support_functions.unpack_the_dataframe( data_frame = data_frame_new) 
        
        # zips a list so its easer to manipulate
        zipped_lists = zip(list_of_data_main, list_of_data_new)
        
        # returns list of the sumed values 
        merged_data = [x + y for (x, y) in zipped_lists]
        
        # creates a dataframe of the merged data
        merged_dataframe = pd.DataFrame(merged_data)
        
        # merges the index and the data togheter, now the dates and 
        the_full_dataframe = pd.DataFrame(index=data_index, data=merged_dataframe.values)
        
        # renames the column to data. 
        the_full_dataframe.columns = ["Data"]
        #
        # replaces the Na's with zero's ( Ment for the change dataframe)
        the_full_dataframe['Data'] = the_full_dataframe['Data'].fillna(0)
        
        return the_full_dataframe
...
class mother_data_object:
    ...
    def clean_lists(self, list_in : list = None ):
        """
