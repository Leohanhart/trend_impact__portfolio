...
class power_stock_object:
    ...
    # controls 
    control_days_for_refresh = 14 # this is the amount of days between the last refresh. This is maily intresseting for dividend data. or shortratio data. 
    ...
    analyses_selected = ["last_calculation_profile_indicator_text","last_calculation_profile_change_text"]
    ...
    def __init__(self,
        ...
        self.synch_object = synch_class.data_synch( path= constants.CORE_DATA_____PATH , subfolder = "stock_data", ticker = stock_ticker, data_extention = "last_download_company_data_refresh")
    ...
    def load_new_stock_info_data(self): 
        ...
        self.future_fair_value_including_dividends = synch_handler.load_data_varible("future_fair")
        # 
        # loads discount on fair. 
        self.future_fair_discount = synch_handler.load_data_varible("future_fair_discount")
        
    def load_stock_data(self, 
                        ignore_last_refresh_date : bool = False,        # is creaded so you can test the refresh methode
                        load_data_for_weekly     : bool = False         # is creaded for analyses modus, runs on the weekly
                        ):
        ...
        #
        # loads the last date
        last_stock_download = synch_handler.load_data_varible("last_download_company_timeserie_data_refresh")
        #
        ...
        if load_data_for_weekly and not date_support.return_true_if_weekly_expired(last_stock_download):
            ...
            self.stock_data = synch_handler.load_data_varible("stock_timeserie_data")
            
            try:
                        
                if not isinstance(self.stock_data, pd.core.frame.DataFrame):
                    
                    raise Exception("Error, booted synch object is not equal pandas dataframe type", "powerstockobject, ")
                    ...
                    raise Exception("Error, booted synch object is not matching column names", "powerstockobject, ")
                
                else:
                    
                    convertion_object = support_functions.time_conversion(stock_data =self.stock_data)
        ...
        if amount_of_dates != 0 or ignore_last_refresh_date == True: 
            ...
            stock_dowload_object = old_supporting_functions.download_stock(self.stock_ticker)
            #
            # Download stock. 
            ...
            synch_handler.save_data_varible(self.stock_data,"stock_timeserie_data")
            #
            # retrieves the current date
            current_date = date_support.return_last_date_object()
            #
            # pushes the last date of the download that has taking place.
            synch_handler.save_data_varible(current_date,"last_download_company_timeserie_data_refresh")
    ...
    def save_new_data(self):
        ...
        # declare dataobject.
        self.data_object = support_functions.load_company_data_yahoo(self.stock_ticker)
        ...
        # if the data is loaded without error it can be parsed. 
            ...
            synch_handler.save_data_varible(self.future_fair_discount,"future_fair_discount")
            ...
            # dowload stockdata
            stock_dowload_object = old_supporting_functions.download_stock(self.company_ticker)
            ...
            synch_handler.save_data_varible(self.last_date_saved,"last_download_company_timeserie_data_refresh")
            
            
            # proveds the date. this is the last time that the object is used. After that it will be a coppie
            synch_handler.save_data_varible(self.last_date_saved,"last_download_company_data_refresh")
            ...
            #report to logbook
            text_for_logbook = "Ticker = "+ self.stock_ticker +" started up at : "+ self.last_date_saved
    ...
    def twitter_module(self, full_test_function = False):
        ...
        data_convertion_object = support_functions.time_conversion(stock_data= self.stock_data)
        ...
        for i in range(0,len(data_convertion_object.time_frames)):
           ...
           data_for_analyeses = data_convertion_object.time_frames[list(data_convertion_object.time_frames.keys())[i]]
           ...
           for x in range(0,len(analyses)):
               ...
               timeframe_of_analyses    =       list(data_convertion_object.time_frames.keys())[i]
               #
               # 
               ...
               #
               # loads old analyses. 
               synch_object = synch_class.data_synch(subfolder = "Data_Folder", ticker = self.stock_ticker, data_extention = extention_of_dict)
               #
               # turns on if class is not intalized before so everthing needs to be prepaired and saved. 
                  ...
                  for y in range(0,len(self.analyses_selected)):
                      ...
                      if new_score != old_score or full_test_function == True:
                          ...
                          text_generator = stock_analyses.twitter_text_generator(name_of_analyeses = analyses[x], 
                             ...
                             ticker_name = self.stock_ticker,                                    # this is the ticker name. 
                             new_recommendation = new_score,                                     # this is the new recoomandtion
                             old_recommendation = old_score,                                     # this is the old recommendation
                             text_input = None ,                                            # this input is for example: News, opionus
                             time_frame = list(data_convertion_object.time_frames.keys())[i],                                             # this is the time frame : Weekly/daily/monthly
                          ...
                          twitter_plot = stock_analyses.plot_generator(
                             ...
                             name_of_analyeses    = analyses[x],   # name of analyes
                             ticker_name = self.stock_ticker,            # ticker name
                             timeframe = list(data_convertion_object.time_frames.keys())[i],              # quite unimportantand  
                          ...
                          # send tweet
                          twitter_object.post_tweet(twitter_text, path_picture)
