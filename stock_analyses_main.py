...
class main_analyeses:
    ...
    use_automatic_time          =   True            # can be used to help set profiles easy. If turend off ignored. 
    ...
    def __init__(self, stock_ticker = "AAPL",                       
        ...
        if timeframe == "D = daily, W = Weekly, M = montly, Q = quarterly, Y = yearly":
            #
        ...
        # sets the timeframe 
              ...
              timeframe == "Daily"      or timeframe == "D"         or      # checks the daily  data 
              timeframe == "Weekly"     or timeframe == "W"         or      # checks the weekly data
              timeframe == "Monthly"    or timeframe == "M"         or      # checks the montly data
              timeframe == "Yearly"     or timeframe == "Y"         or      # checks the yearly data
              timeframe == "Quarterly"  or timeframe == "Q"                 # checks the quater data
    ...
    def load_analyese(self, title_analyses = None ):
        ...
        if title_analyses != None and title_analyses in self.analyses_names:
            ...
            if title_analyses == "MONEYFLOWS": 
               ...
               moneyflow_analyses           = money_flows( stock_ticker=  self.stock_ticker, synchronize= self.synchronize , stock_data = self.stock_data, timeframe=  self.timeframe)
               # loads analyses in dict object
               #moneyflow_analyses.money_flow_analyeses(full_run = True)
               # sets dict object ass main object. 
               self.analyeses_dictionary    = moneyflow_analyses.analyeses_dictionary
            ...
            if title_analyses == "LIQUIDTY":
                ...
                liquidity_analyeses         = liquidity_impact(stock_ticker=  self.stock_ticker, stock_data = self.stock_data, synchronize= self.synchronize , timeframe=  self.timeframe )
                #
                ...
                self.analyeses_dictionary    = liquidity_analyeses.analyeses_dictionary
...
class profiling:
    ...
    def __init__(self, stock_ticker = "", stock_data = None, synchronize = False, timeframe = "D = daily, W = Weekly, M = montly, Q = quarterly, Y = yearly"):
        
        self.stock_data     = stock_data
        self.timeframe      = timeframe    
        
    def return__profile(self, data = None ,  test_function = False, length_for_calculation = 0, return_outcome_as_number = False, based_on_change = False, automatic_use_time_standards = True ):
        ...
        # for test function
        if test_function:
            print("in profiler function the timeseries is converted to dataframe")
        # 
        ...
        if based_on_change == True:
            ...
            # for test function
            if test_function:
                print("in profiler function the timeseries is converted to changed time serie")
                
            list_for_calculation  = data
            #
            # remove 0 in case it doenst work
            list_for_calculation  = [i if i != 0 else 0.01 for i in list_for_calculation]
            #
            # does the calculations nanual 
            list_for_convertation = [a1-a2 for a1, a2 in zip(list_for_calculation[1:], list_for_calculation)]
            #
            # round the numbers. 
            list_for_convertation = [round(num,2) for num in list_for_convertation]
            #
            # convert the session. 
            df_ts = self.convert_data_to_dataframe(data = ts)                
            #
        ...
        if automatic_use_time_standards == True and type(self.timeframe) != None :
        
            # 
            # for test function
            if test_function:
                print("in profiler function the timeseries the periode is converted to the timeframe ")
        ...
        if type(ts) != list:
            ...
            # for test function
            if test_function:
                print("in profiler function the timeseries is converted to list")
            # 
        ...
        if length_for_calculation != 0:
            ...
            # for test function
            if test_function:
                print("in profiler function the timeseries is converted to lenght by imput")
            # 
        ...
        if len(ts) <= 2 :
            ...
            # for test function
            if test_function:
                print("in profiler function the timeseries is errored out because of a len error")
            # 
    ...
    def convert_data_to_dataframe(self, data = None):
        ...
        the_created_dataframe = pd.DataFrame(index=data_index, data=ldataf.values)
        #
...
class twitter_text_generator:
    ...
    analyses_names          =   main_analyeses.analyses_names           # routes the analyeses names 
    text_dictionary         =   {}
    
    def __init__(self, name_of_analyeses = None, 
    ...
    def construct_analyses_texts(self):
        ...
        print("\n\n\nthis is the text 1", self.twitter_text_elements, "\n\nAnd the hashtaged", self.twitter_hashtages)
        time.sleep(5)
        ...
        print("\n\n\nthis is the text 2", self.twitter_text_elements, "\n\nAnd the hashtaged", self.twitter_hashtages)
        time.sleep(5)
        ...
        print("\n\n\nthis is the text 3", self.twitter_text_elements, "\n\nAnd the hashtaged", self.twitter_hashtages)
        time.sleep(5)
        ...
        ### add recommandations ###
        recommandation =   self.return_recommandations(new_recommendation = self.new_recommendation , old_recommendation = self.old_recommendation)
        ...
        self.twitter_text_full = self.twitter_text_full + self.twitter_hastages_text
    ...
    def returns_occation_text(self, imput_occation = None):
        ...
        if type(self.occation) == str and self.occation == "CHANGE_IN_RATE_OF_CHANGE":
    ...
    def return_analyeses_description(self, input_analyeses):
        ...
        if type(self.name_of_analyse) == str and self.name_of_analyse == "LIQUIDTY": 
            # 
            # returns the text. 
        ...
        if type(self.name_of_analyse) == str and self.name_of_analyse == "MONEYFLOWS": 
    ...
    def return_recommandations(self, new_recommendation , old_recommendation ):
        ...
        if new_recommendation != None and old_recommendation != None: 
            #
            # sets text
            text = "\n\nThe old recommandation was \"" + old_recommendation + "\" the new recommandation is \"" + new_recommendation + "\". "
            #
...
class plot_generator:
    ...
    def __init__(self, data = None,             # data of the analyeses
    ...
    def plot_2_frames_with_logo(self, main_title = "Main analyses nane", df1 = None, x1 = None , y1 = None, title1 = "Title of the first plot", 
                  df2 = None , x2 = None, y2 = None, title2 = "The title of the second plot",  
                  xlabel='Date', ylabel='Value', dpi=100,
                  ticker_name = None, analyses_name = None, time_frame = "weekly", path = None):
        ...
        addLogo.set_offset((2551,1280)) # pass the position in a tuple # pass the position in a tuple
        ...
        fig.suptitle(main_title,   fontsize = 32 ,family = ['Times'], y=0.99 , color = "#045BFF")
        ...
        if type(path) != None:
            #plt.show()
            if ticker_name != None and analyses_name != None and time_frame != None:
                
                my_path = os.path.dirname(os.path.abspath(__file__))
                my_file = ticker_name +"_" +analyses_name +"_" +time_frame+"_"+".png"
    ...
    def __construct_picture_name(self):
        ...
        self.name_of_picture = self.ticker_name + ", " + analyses_name_added + ", " + self.timeframe 
    ...
    def generate_change_profile_plot(self):
        ...
        if "indicator_timeserie_raw" in self.analyeses_dictionary:
            ...
            raise Exception('error thrown, no raw indicator data in dictonary', 'plot_generator', self.ticker_name+self.name_of_analyses)
        #
        ...
        if "indicator_timeserie_profile" in self.analyeses_dictionary:
            ...
            data_profile = self.analyeses_dictionary["indicator_timeserie_profile"]
            ...
            raise Exception('error thrown, no profile indicator data in dictonary', 'plot_generator', self.ticker_name+self.name_of_analyses)
...
class twitter_module:
    ...
    def post_tweet(self, text = None, path_photo = None):
        ...
        auth = tweepy.OAuthHandler("wX4lco85Qu9zYqLPchbD4DUov", "G3mKMivifmMJoE8pZGats8fHl8BrMUfMRuhklL6r90aJ0139Qy")
        auth.set_access_token("1227626390171852800-2nSTPjqtaHXk6zO3omtGD6WnUyQDN5", "aGYrAJR6uUkKJYjltSPGkdzsE6wyh9O1nGeJz4Rs1GsU4")
...
class liquidity_analyses_rf:
    ...
    # object profile
    liquidity_profiler_set              = None      # used for the calculations in return_liquidity_profiler
    ...
    def check_or_load_the_timeserie_data(self):
        ...
        self.synch_object = synch_class.data_synch( path= constants.CORE_DATA_____PATH , subfolder = "stock_data", ticker = self.stock_ticker, data_extention = self.class_extention )
        ...
        if self.synch_object.is_data_new == True:
            ...
            self.synch_object.prepair_stock_data_for_static_data(  data = self.stock_data)
            #
            #
            
            
            self.synch_object.prepair_data_in_dataframe(data=self.liquidity_data_set)
            #
        ...
        # turns on if the data is not new. 
            ...
            if self.synch_object.returns_true_if_expired_with_data() == True:
                ...
                # returns a liquidity 
                liquidity_aditiona_data_set = self.liquidity_timeserie(amount_of_days)
    ...
    def check_or_load_the_profiler_data(self):
        ...
        # check if its the first time that the this analyses is done for a stock. 
        self.synch_object_profiler = synch_class.data_synch( path= constants.CORE_DATA_____PATH , subfolder = "stock_data", ticker = self.stock_ticker, data_extention = self.class_extention_profiler )
        ...
        if self.synch_object_profiler.is_data_new == True:
            ...
            # Package the data. 
            # first prepair the stockdata
            self.synch_object_profiler.prepair_stock_data_for_static_data(  data = self.stock_data)
            #
            #
            self.synch_object_profiler.prepair_data_in_dataframe(data=self.liquidity_profiler_set)
            #
        ...
        # turns on if the data is not new. 
            ...
            # check if the data is expided
            if self.synch_object_profiler.returns_true_if_expired_with_data() == True:
                
                #
                # returns the amount of days
                amount_of_days = self.synch_object_profiler.return_amount_of_days()
                #
                # returns a liquidity 
                liquidity_aditiona_data_set = self.liquidity_timeserie_profile_recover(amount_of_days, tial_amount= True)
                #
                # sets the data. 
                self.synch_object_profiler.prepair_data_in_dataframe(liquidity_aditiona_data_set)
                #
                #
                # synches, and automaticly saves the retreived and merged time serie. 
                self.synch_object_profiler.synch_data_timeserie()
                #
                # sets liquide data.
                self.liquidity_profiledata_set_df = self.synch_object_profiler.synchronized_data
                # 
                self.liquidity_profiler_set = self.liquidity_data_profiles.values
                #
                self.liquidity_profiler_set = self.liquidity_profiler_set.tolist()
                # 
                self.liquidity_data_profiles_loaded = True
            ...
            # if not expider just load the retreived data as expired.
                ...
                # sets lid data
                # sets liquide data.
                self.liquidity_profiledata_set_df = self.synch_object_profiler.retreived_data
                # 
                #
                self.liquidity_profiler_set = self.liquidity_profiledata_set_df.values
                #
                #
                self.liquidity_profiler_set = self.liquidity_profiler_set.tolist()
                #
    ...
    def liquidity_timeserie(self,length_for_calculation = 0):
        ...
        if self.liquidity_data_loaded == True:
            ...
            return self.liquidity_data_set
             ...
             workdata = self.stock_data.iloc[len(self.stock_data)-length_for_calculation:len(self.stock_data)]
             calculations = []
             for x in range(0,len(workdata)):
                             
                         outcome = self.liquidty_analyses_with_stock_object_row(workdata.iloc[x])
    ...
    def liquidity_timeserie_profile(self,length_for_calculation = 0, tial = 0, test = False):
        """
        ...
        # starts running if the serie in not loaded yet.
             ...
             for x in range(0,len(workdata)):
                         ...
                         the_last_profile = self.return_liquidity_profile(length_for_calculation = 250,return_outcome_as_number = True,  profiler_timeseries_recoverage= True)
                         # 
    ...
    def liquidity_timeserie_profile_recover(self,length_for_calculation = 0, tial_amount = 0):
        ...
        if self.liquidity_data_profiles_loaded == True:
           ...
           return self.liquidity_data_set
             ...
             workdata = self.stock_data.iloc[len(self.stock_data)-tial_amount:len(self.stock_data)]
             # 
             ...
             if length_for_calculation != 0:
                 ...
                 calculations = self.liquidity_data_set[len(self.liquidity_data_set)-length_for_calculation:len(self.liquidity_data_set)-tial_amount]
                 #
                 ...
                 calculations = self.liquidity_data_set[1:len(self.liquidity_data_set)-tial_amount]
    ...
    def return_liquidity_profile(self, length_for_calculation = 0, return_outcome_as_number = False, profiler_timeseries_recoverage = False,  ):
    ...
    def return_historic_liquidity_profile(self, length_for_calculation = 0, return_outcome_as_number = True, profiler_timeseries_recoverage = True  ):
    ...
    def rolling_liquidity_profiler(self, incomming_data = "incommingdata", length_for_profile = 250,return_outcome_as_number = True):
        ...
        if len(incomming_data) >= length_for_profile:
           ...
           incomming_data = incomming_data[len(incomming_data)-length_for_profile:len(incomming_data)] 
...
class money_flows:
    ...
    def __init__(self, stock_ticker = "", stock_data = None, synchronize = False, timeframe = "D = daily, W = Weekly, M = montly, Q = quarterly, Y = yearly"):
        ...
        if stock_data is not None :
            ...
            self.profiler = profiling(stock_data= stock_data, timeframe= timeframe)
            
        else:
            
            raise Exception("Ticker_None", "This error occures when ticker is Nonetype object")
        ...
        if timeframe == "D = daily, W = Weekly, M = montly, Q = quarterly, Y = yearly":
            ...
            #
            # if timeframe is filled in, it needs to be assinged.
        elif timeframe == "D" or timeframe == "W" or timeframe == "M" or timeframe == "Y" or timeframe == "Q" :
                #
                ...
                if self.timeframe == "W":
                    ...
                    if not isinstance(pd.infer_freq(self.stock_data.index), str):
                        ...
                        raise Exception("Time formate is wrong, analyses dident run")
                        
                    elif "W" not in pd.infer_freq(self.stock_data.index):
                        
                        raise Exception("Time formate is wrong, probably something else the nweekly")
        ...
        data_extention = self.data_extention = self.timeframe + self.name_of_analyeses + "_dictonary"
        
        # creates data object
        self.synch_object = synch_class.data_synch(path= constants.CORE_DATA_____PATH , subfolder = "stock_analyses_data", ticker = self.stock_ticker, data_extention = data_extention)
        ...
        elif not self.synch_object.is_data_new and type(timeframe) != None and synchronize: 
            
            if not self.synch_object.return_last_modification_file(view = timeframe, experiation_bool = True):
                ...
                if not self.synch_object.check_incomming_dict_legit(self.analyeses_dictionary):
                    self.first_run = True
                    # starts full analyses. 
                ...
                # set indicator
                indicator  = self.analyeses_dictionary['indicator_timeserie_raw']
                ...
                amount_of_expired = self.synch_object.time_bewteen_time_series(indicator, self.stock_data, self.timeframe   )
                ...
                if amount_of_expired == 0:
                    ...
                    self.money_flow_analyeses(full_run = True, tail_amount = amount_of_expired )
    ...
    def money_flow_with_row(self, data_row = None): 
        ...
        #print(x.index)
        if "Open" in x.index  and "High" in x.index  and "Low" in x.index and "Close" in x.index  and "Volume" in x.index:
        ...
        try:
            ...
            money_flow_multiplier   = ((price_close - price_low) - (price_high - price_close)) / (price_high - price_low)
    ...
    def money_flow_analyeses(self, full_run = False, test_function = False, tail_amount = 0 , length_for_calculation = 0, return_last_varible_only = False, as_profile = False,  based_on_change = False, return_as_number = True):
        """
        ...
        if self.synchronize and not self.first_run and tail_amount > 0:
            ...
            #
            # loads dict data
            data_from_dict = self.analyeses_dictionary["indicator_timeserie_raw"]
            #
            # pushed converted data into analyses list
            self.calculations = data_converter.convert_nested_analeses_dict_data_to_list(data_from_dict)
        ...
        if self.synchronize and not self.first_run and tail_amount > 0:
            ...
            #
            # loads dict data
            data_from_dict = self.analyeses_dictionary["indicator_timeserie_profile"]
            #
            # pushes the data in list, just like above here.
            self.calculations_profile = data_converter.convert_nested_analeses_dict_data_to_list(data_from_dict)
        ...
        if self.synchronize and not self.first_run and tail_amount > 0:
            ...
            #
            # loads dict data
            data_from_dict = self.analyeses_dictionary["indicator_timeserie_profile_change"]
            #
            # pushes the data in list, just like above here.            
            self.calculations_profile_change = data_converter.convert_nested_analeses_dict_data_to_list(data_from_dict)
        ...
        if tail_amount == 0: # and synch and not first run                                                   # this means that the synchronisation is turned on.                  
            for x in range(0,len(workdata)):
                        ...
                        if as_profile == True and based_on_change != True and return_last_varible_only != True or full_run == True :
                            ...
                            outcome_profile = self.return__profile( data = self.calculations, test_function = test_function , length_for_calculation = length_for_calculation , return_outcome_as_number = return_as_number  )
                            # 
                        ...
                        if as_profile == True and based_on_change == True and return_last_varible_only != True or full_run == True :
                            ...
                            #
                            #
                            self.outcome_profile_change = self.return__profile( data = self.calculations, test_function = test_function , length_for_calculation = length_for_calculation , return_outcome_as_number = return_as_number ,   based_on_change = True )
                            #
                            # if there is no thing marked, it can just roll 
                            self.calculations_profile_change.append(self.outcome_profile_change)
                            #
                            #
                            #
                            # if function is tested
        ...
        if full_run == True :
            ...
            self.analyeses_dictionary["last_calculation_indicator"] = self.calculations[len(self.calculations)-1]
            #
            # receives the last profile - as text NORMAL/HIGH ect( Its calculated so thats why)
            self.analyeses_dictionary["last_calculation_profile_indicator_text"] = self.return__profile( data = self.calculations , based_on_change = False, return_outcome_as_number = False, automatic_use_time_standards = self.use_automatic_time)
            #
            # receives the last profile - as a number 
            self.analyeses_dictionary["last_calculation_profile_indicator_number"] = self.return__profile( data = self.calculations , based_on_change = False, return_outcome_as_number = True, automatic_use_time_standards = self.use_automatic_time)
            #
            # reveices the last profile based on change, marked as a text. 
            self.analyeses_dictionary["last_calculation_profile_change_text"] = self.return__profile( data = self.calculations , based_on_change = True, return_outcome_as_number = False, automatic_use_time_standards = self.use_automatic_time)
            #
            # receives the last profile based on change number.
            self.analyeses_dictionary["last_calculation_profile_change_number"] = self.return__profile( data = self.calculations , based_on_change = True, return_outcome_as_number = True , automatic_use_time_standards = self.use_automatic_time)
            #
            # timeserie of the indicator data
            self.analyeses_dictionary["indicator_timeserie_raw"]    = self.convert_data_to_dataframe(self.calculations)
            #
            # timeserie of the indicator data
            self.analyeses_dictionary["indicator_timeserie_profile"]    = self.convert_data_to_dataframe(self.calculations_profile)
            #
            # time serie
            self.analyeses_dictionary["indicator_timeserie_profile_change"]    = self.convert_data_to_dataframe(self.calculations_profile_change)
            #
            ...
            #
            # convert the session. 
            indicator_timeserie_change = self.convert_data_to_dataframe(data = list_for_convertation)
            # removes NA's
            indicator_timeserie_change = indicator_timeserie_change.fillna(0)
            
            
            self.analyeses_dictionary["indicator_timeserie_change"]    = indicator_timeserie_change
            #
            # saves synch
            ...
            if self.synchronize:
                ...
                if self.first_run == True:
                    ...
                    # adds data
                    self.synch_object.synchronized_data =  self.analyeses_dictionary
        ...
        # ( Loose varible ) returns loosse and last profile as number( This can be used for the dashboard). 
        elif as_profile == True and based_on_change != True and return_last_varible_only == True and return_as_number != True:
            ...
            outcome = self.return__profile(data= self.calculations , based_on_change = False, return_outcome_as_number = False, automatic_use_time_standards= self.use_automatic_time)
            #
            #
            return outcome 
        # 
        # (Loos varible) returns loose profile as text
            ...
            # 
            # returns the last profile, as text 
            outcome = self.return__profile( data = self.calculations , based_on_change = False, return_outcome_as_number = True, automatic_use_time_standards = self.use_automatic_time)
            #
            #
            return outcome 
...
class liquidity_impact: 
    ...
    def __init__(self, stock_ticker = "", stock_data = None, synchronize = False, timeframe = "D = daily, W = Weekly, M = montly, Q = quarterly, Y = yearly"):
        ...
        if timeframe == "D = daily, W = Weekly, M = montly, Q = quarterly, Y = yearly":
            ...
            #
            # if timeframe is filled in, it needs to be assinged.
        elif timeframe == "D" or timeframe == "W" or timeframe == "M" or timeframe == "Y":
                #
                # if it fits, it assings. 
                self.timeframe = timeframe
        ...
        # creates name extrention
        data_extention = self.data_extention = self.timeframe + self.name_of_analyses + "_dictonary"
            ...
            if not self.synch_object.return_last_modification_file(view = timeframe, experiation_bool = True):
                ...
                if amount_of_expired == 0:
                    ...
                    try: 
                        ...
                        self.liquidity_impact_analyeses(full_run = True, tail_amount = amount_of_expired )
    ...
    def liquidity_impact_analyeses(self, full_run = False, test_function = False, tail_amount = 0 , length_for_calculation = 0, return_last_varible_only = False, as_profile = False,  based_on_change = False, return_as_number = True):
        """
        ...
        if tail_amount == 0:
            ...
            for x in range(0,len(workdata)):
                        ...
                        if as_profile == True and based_on_change != True and return_last_varible_only != True or full_run == True :
                            ...
                            #
                            # returns the profile if the a
                            outcome_profile = self.profiler.return__profile( data = self.calculations, test_function = test_function , length_for_calculation = length_for_calculation , return_outcome_as_number = return_as_number  )
                            # 
                            # adds the calculation to the profile.
                            self.calculations_profile.append(outcome_profile)
                        ...
                        if as_profile == True and based_on_change == True and return_last_varible_only != True or full_run == True :
                            ...
                            #
                            #
                            self.outcome_profile_change = self.profiler.return__profile( data = self.calculations, test_function = test_function , length_for_calculation = length_for_calculation , return_outcome_as_number = return_as_number ,   based_on_change = True )
                            #
                            # if there is no thing marked, it can just roll 
                            self.calculations_profile_change.append(self.outcome_profile_change)
                            #
        ...
        if full_run == True :
            ...
            #
            # receives the last profile - as text NORMAL/HIGH ect( Its calculated so thats why)
            self.analyeses_dictionary["last_calculation_profile_indicator_text"] = self.profiler.return__profile( data = self.calculations , based_on_change = False, return_outcome_as_number = False, automatic_use_time_standards = self.use_automatic_time)
            #
            # receives the last profile - as a number 
            self.analyeses_dictionary["last_calculation_profile_indicator_number"] = self.profiler.return__profile( data = self.calculations , based_on_change = False, return_outcome_as_number = True, automatic_use_time_standards = self.use_automatic_time)
            #
            # reveices the last profile based on change, marked as a text. 
            self.analyeses_dictionary["last_calculation_profile_change_text"] = self.profiler.return__profile( data = self.calculations , based_on_change = True, return_outcome_as_number = False, automatic_use_time_standards = self.use_automatic_time)
            #
            # receives the last profile based on change number.
            self.analyeses_dictionary["last_calculation_profile_change_number"] = self.profiler.return__profile( data = self.calculations , based_on_change = True, return_outcome_as_number = True , automatic_use_time_standards = self.use_automatic_time)
            #
            # timeserie of the indicator data
            self.analyeses_dictionary["indicator_timeserie_raw"]    = self.convert_data_to_dataframe(self.calculations)
            #
            ...
            #
            # creates the data_serie for change 
            indicator_timeserie_change = self.convert_data_to_dataframe(self.calculations)
            # changes value to value of change
            indicator_timeserie_change = indicator_timeserie_change.pct_change()
            # removes NA's
            indicator_timeserie_change = indicator_timeserie_change.fillna(0)
