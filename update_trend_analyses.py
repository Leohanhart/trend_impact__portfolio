...
class update_kaufman_support(object):
    ...
    def sleep_until(target_hour, target_minute, target_second):
        ...
        def return_profiles_data(
            ...
            if last_dp_trend > 0:
                ...
                cur_price = round(float(data_total.Close.tail(1)), 2)
                max_yield = round(
                    (((min_price / startprice) - 1) * 100) * -1, 2
                )
                current_yield = round(
            ...
            output_data[
                "volatility"
            ] = update_kaufman_support.retrun_volatiltiy(
    ...
    class update_archive_kaufmal:
        ...
        def __init__(
            ...
            if len(stock_data) < 30:
                ...
                raise Exception(
                    "DATA_ERROR", "Data is not long enough ", ticker
                )
        ...
        def __init__(
            ...
            for i in range(
                ...
                if (
                    ...
                    and self.check_if_duration_is_same(old_model, model)
                    == False
                ):
    ...
    class update_trend_performance:
        ...
        def create_performance_details(self, dataframe, name=""):
            ...
            # total mean
            data[
                "total_average_return_" + name
            ] = ty_total_mean_return = round(df.current_yield.mean(), 2)
            ...
            try:
                ty_perctage_positive_trades = (
                    round(ty_amount_positive_trades / ty_amount_of_trades, 2)
                    * 100
                )
            except:
            ...
            try:
                ...
                ty_perctage_negative_trades = (
                    round(ty_amount_negative_trades / ty_amount_of_trades, 2)
                    * 100
                )
        ...
        def aggegrate_data(self, df):
            ...
            df_agg = df.groupby(
                (df["trend"].shift() != df["trend"]).cumsum()
            ).agg(
                {
                    "start_date": "first",
                    "end_date": "last",
    ...
    class portfolio_constructor_manager:
        ...
        def __init__(
            ...
            self.portfolio_strat_high_sharp_stocks = (
                self.process__stocks__to__df(
            ...
            balances_low_vol = list(
                self.low_vol_frame.balance.to_dict().values()
            )
            # sets data balanced
            ...
            low_vol_expected_return = (
                low_volatile_summed.pct_change().mean() * 100
            )
        ...
        def return_max_drawdown(
            ...
            if position_side == 1:
                ...
                Daily_Drawdown = (
                    stock__data__frame["Adj Close"] / Roll_Max - 1.0
                )
                ...
                # Next we calculate the minimum (negative) daily drawdown in that window.
                # Again, use min_periods=1 if you want to allow the expanding window
                Max_Daily_Drawdown = Dd_test.rolling(
                    window, min_periods=1
                ).min()
                # return in right format,
                if not return_time_serie:
        ...
        def process__portfolio_specs__to__df(self, df: dict, UUid: str):
            ...
            df["total_amount_stocks"] = len(
                self.portfolio_strat_low_vol_stocks
            )
